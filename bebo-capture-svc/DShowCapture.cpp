...
#include "elgato/IVideoCaptureFilter.h"
...
HRESULT DShowCapture::GetDeviceSettings() {
    ...
    if (elgato.Get()->GetDevicePresent(&device_present) == S_OK) {
        info("device present: %d", device_present);
    }

    ElgatoGameCapture::VIDEO_CAPTURE_FILTER_DEVICE_TYPE device_type;
    if (elgato.Get()->GetDeviceType(&device_type) == S_OK) {
        info("device type: %d", device_type);
    }

    ElgatoGameCapture::VIDEO_CAPTURE_FILTER_SETTINGS_EX device_settings = { 0 };
    ...
    if (elgato.Get()->GetSettingsEx(&device_settings) == S_OK) {
        ...
        info("%s \
            ...
            device_settings.Settings.profile,
            device_settings.Settings.useAnalogAudioInput,
            device_settings.Settings.hdmiColorRange,
            device_settings.Settings.brightness,
            device_settings.Settings.contrast,
            device_settings.Settings.saturation,
            device_settings.Settings.hue,
            device_settings.Settings.analogAudioGain,
            device_settings.Settings.digitalAudioGain,
            device_settings.Settings.preserveInputFormat,
            device_settings.Settings.stretchStandardDefinitionInput,
            device_settings.enableFullFrameRate,
            device_settings.deviceIndex,
            device_settings.audioGainshowDb,
            device_settings.lastSkippedUpdate,
            device_settings.checkForBetaVersions);
...
bool DShowCapture::GetFrame(IMediaSample ** left_hand_sample)
{
    ...
    if (first_frame && media_sample_queue_.size() < 15) {
        return false;
    }
    first_frame = false;
