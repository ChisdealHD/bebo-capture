...
public:
	...
	bool wait_for_and_pop(T &popped_item, long millis) {
		std::unique_lock<std::mutex> lock(m_);
		while (queue_.empty())
		{
			if (data_cond_.wait_for(lock, std::chrono::milliseconds(millis)) == std::cv_status::timeout) {
				return false;
			}
			//  This 'while' loop is equal to
			//  data_cond_.wait(lock, [](bool result){return !queue_.empty();});
	...
	/// wait till an item is available and try again
	void wait() {
		std::unique_lock<std::mutex> lock(m_);
		while (queue_.empty())
		{
